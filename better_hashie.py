...
class hashie(plugins.Plugin):
    ...
    __description__ = '''
                        ...
                        The files are saved in Hashcat format:
                          - EAPOL/PMKID hashes are saved as *.22000/*.22001
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        with self.lock:
            ...
            elif self._writeFILE(filename, access_point):
    ...
    def _writeFILE(self, fullpath, apJSON):
        ...
        if os.path.isfile(fullpathNoExt + '.22000'):
            logging.debug('[hashie] [+] Success: {}.22000 created'.format(filename))
            return True
        else: #make a raw dump
            if os.path.isfile(fullpathNoExt + '.22000'):
                if self._repairPMKID(fullpath, apJSON) == False:
                    logging.debug('[hashie] [-] Fail: {}.22000 could not be repaired'.format(filename))
                    return False
                else:
                    logging.debug('[hashie] [+] Success: {}.22000 repaired'.format(filename))
                    return True
            else:
                logging.debug('[hashie] [-] Could not attempt repair of {} as no file was created'.format(filename))
                return False
    ...
    def _repairPMKID(self, fullpath, apJSON):
        ...
        with open(fullpathNoExt + '.22000','r') as tempFileA:
        ...
        if apJSON != "": 
            ...
            result = subprocess.getoutput('hcxpcapngtool -o /tmp/{} {} >/dev/null 2>&1'.format(filename,fullpath))
        ...
        if clientString:
            for line in clientString:
                if line.split(':')[0] == hashString.split(':')[1]: #if the AP MAC pulled from the JSON or tcpdump output matches the AP MAC in the raw 16800 output
                    ...
                    if (len(hashString.split(':')) == 4) and not (hashString.endswith(':')):
                        with open(fullpath.split('.')[0] + '.22000','w') as tempFileC:
            ...
            os.remove(fullpath.split('.')[0] + '.22000')
            return False
    ...
    def _process_stale_pcaps(self, handshake_dir):
        ...
        for num, handshake in enumerate(handshakes_list):
            ...
            if not os.path.isfile(fullpathNoExt + '.22000'): #if no 16800, try
                if self._writeFILE(handshake, ""):
