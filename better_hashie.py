...
class hashie(plugins.Plugin):
    ...
    def _writeEAPOL(self, fullpath):
        ...
        result = subprocess.getoutput('hcxpcaptool -x {}.2500 {} >/dev/null 2>&1'.format(fullpathNoExt,fullpath))
        if os.path.isfile(fullpathNoExt +  '.2500'):
    ...
    def _writePMKID(self, fullpath, apJSON):
        fullpathNoExt = fullpath.split('.')[0]
        filename = fullpath.split('/')[-1:][0].split('.')[0]
        result = subprocess.getoutput('hcxpcaptool -z {}.16800 {} >/dev/null 2>&1'.format(fullpathNoExt,fullpath))
        if os.path.isfile(fullpathNoExt + '.16800'):
            logging.debug('[hashie] [+] PMKID Success: {}.16800 created'.format(filename))
            return True
        else: #make a raw dump
            result = subprocess.getoutput('hcxpcaptool -X {}.16800 {} >/dev/null 2>&1'.format(fullpathNoExt,fullpath))
            if os.path.isfile(fullpathNoExt + '.16800'):
    ...
    def _repairPMKID(self, fullpath, apJSON):
        ...
        if apJSON != "": 
            ...
            result = subprocess.getoutput('hcxpcaptool -z /tmp/{} {} >/dev/null 2>&1'.format(filename,fullpath))
